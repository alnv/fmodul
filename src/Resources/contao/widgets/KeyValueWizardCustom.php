 ...
 * @package FModule
 */
class KeyValueWizardCustom extends \Widget
{
  ...
  <thead>
    <tr>
      <th>'.$GLOBALS['TL_LANG']['tl_module']['ow_key'].'</th>
  ...
  <tbody class="sortable" data-tabindex="'.$tabindex.'">';
        ...
        for ($i=0, $c=count($this->varValue); $i<$c; $i++)
        {
            ...
            {
                $currentVal = specialchars($this->varValue[$i]['key']);
                $val = specialchars($value['value']);
                $label = $value['label'];
                $strOptions .= '<option value="' . $val . '" '. ($currentVal == $val ? 'selected' : '') .'>'. $label .'</option>';
            }
    ...
    <tr>
      ...
      <td style="white-space:nowrap;padding-left:3px">';
            ...
            foreach ($arrButtons as $button)
            {
                ...
                {
                    $return .= '<a href="'.$this->addToUrl('&amp;'.$strCommand.'='.$button.'&amp;cid='.$i.'&amp;id='.$this->currentRecord).'"' . $class . ' title="'.specialchars($GLOBALS['TL_LANG']['MSC']['ow_'.$button]).'" onclick="FModule.keyValueWizardCustom(this,\''.$button.'\',\'ctrl_'.$this->strId.'\');return false">'.\Image::getHtml($button.'.gif', $GLOBALS['TL_LANG']['MSC']['ow_'.$button]).'</a> ';
                }
            }
