...
class FModuleOrderByWizard extends \Widget {
    ...
    public function generate() {
        ...
        if ( !$this->orderByTablename ) {
            ...
            if ( $objModule->numRows && $objModule->f_select_module ) {

                $this->orderByTablename = $objModule->f_select_module;
            }
        }
                          ...
                          <td style="white-space:nowrap;padding-left:3px">';
                                ...
                                foreach ( $arrButtons as $button ) {
                                    ...
                                    if ( $button == 'drag' ) {
                                        ...
                                        $return .= '<a href="'.$this->addToUrl('&amp;'.$strCommand.'='.$button.'&amp;cid='.$i.'&amp;id='.$this->currentRecord).'"' . $class . ' title="'.specialchars($GLOBALS['TL_LANG']['MSC']['ow_'.$button]).'" onclick="FModule.FModuleOrderByWizard(this,\''.$button.'\',\'ctrl_'.$this->strId.'\');return false">'.\Image::getHtml($button.'.gif', $GLOBALS['TL_LANG']['MSC']['ow_'.$button]).'</a> ';
                                    }
                                }
                                $return .= '</td>
                        </tr>';
